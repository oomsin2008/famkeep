 ...
 *  origin. `deletable` (Tasks list only) appends an inline delete. In
 *  `selectMode` the row body toggles selection instead of navigating. */
export function TaskRow({
  ...
  return (
    <div
      className={[
        "fk-row flex min-h-11 items-start gap-2 p-3.5 transition-colors",
    ...
    >
      {selectMode ? (
        <button
          ...
          aria-label={`เลือก ${task.title}`}
          className="flex min-w-0 flex-1 items-start gap-3 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40"
        >
          <span
            aria-hidden
            className={[
